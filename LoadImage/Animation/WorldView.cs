...
namespace dotNetStiEditor
{
	...
	public class WorldView
	{
        ...
        {
            pBackground = background;
            pBackBuffer = new Bitmap(pClientArea.Width, pClientArea.Height);
        }
		...
		public void RenderAll(Graphics clip,Rectangle area)
		{
			...
			for(int count=0;count<pSprites.Count;count++)
			{
                Sprite sprite = pSprites.Item((int)pSprites.SortedByZ[count]);
				Rectangle dest=sprite.oDestRect;
				dest.Offset(-pOrigin.X,-pOrigin.Y);
                back.DrawImage(sprite.oCanvas.PictureFile[sprite.oFrame].Bm, dest);
					//sprite.oSourceRect,GraphicsUnit.Pixel);
			}
			//clear it up
			pRequestList.Clear();
		...
		public void RenderEachClipped()
		{
			...
			for(int count=0;count<pSprites.Count;count++)
			{
				if(pRequestList.Contains((int)pSprites.SortedByZ[count]))
				{
                    ...
                    Sprite sprite = pSprites.Item((int)pSprites.SortedByZ[count]);
                    Rectangle dest = sprite.oDestBgrRect;
					source=dest;
					dest.Offset(-pOrigin.X,-pOrigin.Y);
					if (pBackground != null)
						back.DrawImage(pBackground, dest, source, GraphicsUnit.Pixel);
					//if(DrawTile)
						//back.DrawImage(dotNetStiEditor.Properties.Resources.Tile, sprite.TilePosition);
					//uncomment to see clips
					//back.DrawImage(pBackground,dest,new Rectangle(0,0,10,10),GraphicsUnit.Pixel);
				}
			}
			for(int count=0;count<pSprites.Count;count++)
			{
				if(pRequestList.Contains((int)pSprites.SortedByZ[count]))
				{
					...
					dest.Offset(-pOrigin.X,-pOrigin.Y);
                    back.DrawImage(sprite.oCanvas.PictureFile[sprite.oFrame].Bm, dest);
                        //(pSprites.Item((int)pSprites.SortedByZ[count]).oCanvas.PictureFile,dest,
		...
		public void Render()
		{
			...
			for(int count=0;count<pSprites.Count;count++)
			{
				...
				if(pRequestList.Contains((int)pSprites.SortedByZ[count]))
				{
                    ...
                    Sprite sprite = pSprites.Item((int)pSprites.SortedByZ[count]);
                    dest = sprite.oDestRect;
					dest.Offset(-pOrigin.X,-pOrigin.Y);
                    back.DrawImage(sprite.oCanvas.PictureFile[sprite.oFrame].Bm, dest);
                        //(pSprites.Item((int)pSprites.SortedByZ[count]).oCanvas.PictureFile,
                        //dest,
                        //pSprites.Item((int)pSprites.SortedByZ[count]).oSourceRect,GraphicsUnit.Pixel);
				}
			}
		...
		private void CreateClips()
		{
			...
			for(int count=0;count<pRequestList.Count;count++)
			{
				pClipRectangle.Add(pSprites.Item((int)pRequestList[count]).oDestBgrRect); //.oDestRect);
